...
class ArrivalGenerator(metaclass=Singleton):
    ...
    @abstractmethod
    def generate_arrival(self, prev_task, task_id):
        """
...
class ExperimentGenerator(ArrivalGenerator):
    ...
    def generate_arrival(self, prev_time, task_id: str) -> Arrival:
        """
        ...
        inter_arrival_ticks = prev_time + np.random.poisson(lam=job.arrival_statistic)
        ...
        while self._alive and time.time() - self.start_time < duration:
            ...
            for entry in self._tick_list:
                ...
                if entry.ticks <= 0:
                    ...
                    # For now commented to generate only the jobs specified
